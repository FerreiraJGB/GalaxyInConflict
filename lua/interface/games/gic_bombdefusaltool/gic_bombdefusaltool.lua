        ...
        if onFossilId then
          ...
          if self.onFossilTime >= self.minOnFossilTime then
            if not world.getObjectParameter(onFossilId, "inUse") then
              local configData = root.assetJson("/interface/games/gic_bombdefusalgame/gic_bombdefusalgamegui.config")
...
function onFossil()
  ...
  if objectId and root.itemHasTag(world.entityName(objectId), "gic_cain_teleporterbomb") then
    return objectId
  end
  return false
end
