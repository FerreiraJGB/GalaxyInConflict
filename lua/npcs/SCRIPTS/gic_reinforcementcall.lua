...
function init()
  ...
  self.help = 0 --This is to keep track of whether or not the NPC has called the Space Police
  self.random = math.random(1,2) --Chance to call Police, because balance I guess. Every call gets 2 helpers so 1/5 is okay, right???
  self.randomhealth = math.random(50,95)/100 --At what health percentage the NPC calls for help. Some mistakes should be forgiven, right? Some randomness for fun.
...
function update(dt)
    ...
    for _, notification in ipairs(damageNotifications) do
      if notification.healthLost > self.minTriggerDamage and status.resourcePercentage("health") < self.randomhealth and self.help == 0 and notification.sourceEntityId ~= notification.targetEntityId and self.random == 1 then --Checks if the player has damaged the NPC and stuff.
		if math.random(1,1) == 1 then --1 in 4 chance of spawning Coptop team.
	  	world.spawnNpc({mcontroller.xPosition() + 2, mcontroller.yPosition()}, "human", "ue_reinforcementunit", world.threatLevel()-3) --Player attacked the NPC, call the freakin' Space Police!!!
		world.spawnNpc({mcontroller.xPosition() - 2, mcontroller.yPosition()}, "human", "ue_reinforcementunit", world.threatLevel()-3) --Y'know what, call 2 of them!
		end
...
function offeredQuestStarted(questArc)
  if entity.damageTeam().type ~= "enemy" then
    storage.preQuestDamageTeam = entity.damageTeam()
    npc.setDamageTeam({
        type = "enemy",
        team = 2 -- Friendly NPCs always on team 1... Hah fuck no it's the enemy!
      })
  end
end
