...
function update(dt)
  ...
  if self.loungeShuttlecraftId ~= nil then 
	...
	-- these NPCs will try to board turrets; please note that there's no pathfinding yet from these NPCs to drive them to nearest turret!
	local vehicleList = world.entityQuery(entity.position(), 3, {includedTypes = {"vehicle"}, order = "nearest" })
	if vehicleList[1] ~= nil then 
		local uuid = 0 --this is just here as a reminder for the purpose of this variable
		world.sendEntityMessage(vehicleList[1], "gic_boardTurret", uuid, entity.id())
	end
  end
