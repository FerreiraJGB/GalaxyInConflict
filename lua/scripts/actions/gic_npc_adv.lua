-- "advanced" more like hijacking vanilla code for schenanigans
...
-- custom code
function hasBackupSheathed(args, board)
  if self.sheathedPrimary == nil then return false end
  
  if self.sheathedPrimary.parameters and self.sheathedPrimary.parameters.backupWeapon then return true end
  return root.itemHasTag(self.sheathedPrimary.name, "backup")
end

function sheathedBackupRange(args, board)
  if self.sheathedPrimary == nil then return false end
  
  if self.sheathedPrimary.parameters and self.sheathedPrimary.parameters.backupWeaponRange then 
  --sb.logInfo(sb.printJson(self.sheathedPrimary.parameters.backupWeaponRange)); 
  return true, {range = self.sheathedPrimary.parameters.backupWeaponRange} end
  return true, {range = 0}
end

function hasBackup(args, board)
  if self.primary == nil then return false end
  
  if self.primary.parameters and self.primary.parameters.backupWeapon then 
  --sb.logInfo(sb.printJson(self.primary.parameters.backupWeapon)); return true end
  return true end
  return root.itemHasTag(self.primary.name, "backup")
end


function backupRange(args, board)
  if self.primary == nil then return false end
  
  if self.primary.parameters and self.primary.parameters.backupWeaponRange then return true, {range = self.primary.parameters.backupWeaponRange} end
  return true, {range = 0}
end
