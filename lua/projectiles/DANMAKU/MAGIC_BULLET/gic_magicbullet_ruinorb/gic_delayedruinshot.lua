...
function destroy()
  if self.target and projectile.sourceEntity() and world.entityExists(projectile.sourceEntity()) then
    ...
    world.spawnProjectile("gic_magicbullet_ruinorb_homing_05s", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 50, power = projectile.getParameter("power"), damageTeam = {type = "indiscriminate"}})
