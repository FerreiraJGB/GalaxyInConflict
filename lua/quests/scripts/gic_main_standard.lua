...
function update(dt)
  if self.timer > 0 then
    ...
    --seconds; i dunno if a quest updating once per second is too slow, but keeping it at this number to reduce lag better (i.e. stacked w boss rush quests, repeatables, etc)
	self.timer = 1 + math.random() * dt * 10
	-- randomness is to add minor variation so not all quests tick simultaneously
  end
