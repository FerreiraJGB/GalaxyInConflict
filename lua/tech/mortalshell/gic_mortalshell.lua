...
function update(args)
  ...
  if self.specialLast and not args.moves["special1"] then
    attemptActivation()--self.forceTimer = nil
  end
  ...
  if self.active then
	...
	self.parryTimer = self.parryTimer - args.dt
	
	if self.parryTimer > 0 then
		animator.setGlobalTag("borderDirectives","?border=1;FFFFFF;")
	else
		animator.setGlobalTag("borderDirectives","")
	end
  end
...
function activate()
	...
	status.addEphemeralEffect("gic_parryshield_mortalshell",1.5)
  ...
  if not self.active then
	...
	if facing < 0 then
		animator.setGlobalTag("flipDirectives","?flipx")
	else
...
function deactivate()
	...
	status.removeEphemeralEffect("gic_parryshield_mortalshell")
