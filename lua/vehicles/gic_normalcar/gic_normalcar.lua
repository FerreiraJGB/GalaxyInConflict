  ...
  if facingDirection < 0 then
    ...
    if vehicle.controlHeld("drivingSeat", "primaryFire") then
      if self.fireTimer2 <= 0 then
        world.spawnProjectile(mechProjectile2, vec2.add(mcontroller.position(), animator.partPoint("frontGun2", "firePoint2")), entity.id(), {math.cos(aimAngle), math.sin(aimAngle)}, false, mechProjectile2Config)
