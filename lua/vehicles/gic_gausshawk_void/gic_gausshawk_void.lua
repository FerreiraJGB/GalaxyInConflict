...
function init()
  ...
  --LEFLS STUFF
   ...
   shuttleProjectile2 = config.getParameter("shuttleProjectile2")
   shuttleProjectile2Config = config.getParameter("shuttleProjectile2Config")
   shuttleFireCycle2 = config.getParameter("shuttleFireCycle2")
...
function controls2()
 ...
 if (vehicle.controlHeld("drivingSeat","altFire")) then
   if self.fireTimer2 <= 0 then
    world.spawnProjectile(shuttleProjectile2, vec2.add(mcontroller.position(), animator.partPoint("frontGun2", "firePoint2")), entity.id(), {math.cos(aimAngle), math.sin(aimAngle)}, false, shuttleProjectile2Config)
    self.fireTimer2 = self.fireTimer2 + shuttleFireCycle2
    ...
    if oldFireTimer2 > shuttleFireCycle2 / 2 and self.fireTimer2 <= shuttleFireCycle2 / 2 then
      world.spawnProjectile(shuttleProjectile2, vec2.add(mcontroller.position(), animator.partPoint("backGun2", "firePoint2")), entity.id(), {math.cos(aimAngle), math.sin(aimAngle)}, false, shuttleProjectile2Config)
      animator.setAnimationState("backFiring2", "fire")
    end
   end
 end
end
