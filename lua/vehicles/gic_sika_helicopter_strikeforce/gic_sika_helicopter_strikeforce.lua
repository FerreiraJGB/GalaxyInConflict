...
function init()
  ...
  self.autocannonReloadTimer = 0.25
  
  self.autocannonReloadTimerDefault = 0.25 --1.0s to reload an autocannon shell
end
...
function move()
  ...
  if self.driver and self.rocketsLoaded < 9 then
	...
	if self.rocketReloadTimer <= 0 then
		...
		animator.setSoundVolume("rocketpodReload",1)
  ...
  if self.driver and self.autocannonAmmo < 12 then
	...
	if self.autocannonReloadTimer <= 0 then
		...
		animator.setSoundVolume("autocannonReload",1)
...
function controls()
 ...
 --if self.fireTimer >= 0 then self.fireTimer = self.fireTimer - script.updateDt()end
  ...
  local podLimit = 2 * math.pi / 180 --2 degrees of movement
  local aimAngle = self.angle
 ...
 local primaryHeld = vehicle.controlHeld("drivingSeat","primaryFire")
 
 if primaryHeld and (self.rocketsLoaded > 0 or self.autocannonAmmo > 0) then
	local facingMulti = 0
	if self.facingDirection == -1 then facingMulti = 1 end
	
	local dirVector = {math.cos(self.angle+math.pi*facingMulti), math.sin(self.angle+math.pi*facingMulti)}
	
	-- rockets
	if self.rocketpodTimer <= 0 and self.rocketsLoaded > 0 then
		world.spawnProjectile(config.getParameter("rocketpodProjectile"), vec2.add(mcontroller.position(), animator.partPoint("rocketPodPosition", "firePoint")), entity.id(), dirVector, false, config.getParameter("rocketpodProjectileConfig"))
		self.rocketsLoaded = self.rocketsLoaded - 1
	...
	if self.autocannonTimer <= 0 then
		world.spawnProjectile(config.getParameter("autocannonProjectile"), vec2.add(mcontroller.position(), animator.partPoint("autocannonPosition", "firePoint")), entity.id(), dirVector, false, config.getParameter("autocannoProjectileConfig"))
		self.autocannonAmmo = self.autocannonAmmo - 1
