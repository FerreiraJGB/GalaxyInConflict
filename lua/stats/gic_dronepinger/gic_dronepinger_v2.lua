...
function init()
  ...
  self.statusEffect = config.getParameter( "statusEffect" )
  self.statusDuration = config.getParameter( "statusDuration" )
  self.reloadTimer = 0
...
function effectTargets()
  ...
  for _, candidate in ipairs(candidates) do
    if world.entityCanDamage(self.sourceEntity, candidate) then
		...
		world.sendEntityMessage(candidate, "applyStatusEffect", self.statusEffect, self.statusDuration, self.sourceEntity)
	end
  end
end
