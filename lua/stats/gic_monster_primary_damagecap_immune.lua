...
function init()
  self.overrideStatResistances = {
	--ews_smallarmsResistance = 1.0,
	--ews_heavyarmsResistance = 1.0
  }
...
function applyDamageRequest(damageRequest)
  local dodgeStat = status.stat("gic_dodge_stat")
	--sb.logInfo("--")
  ...
  and damageRequest.damage ~= 0 then
	...
	if statRoll <= dodgeStat then
		...
		status.addEphemeralEffect("gic_npcdodge")
		return {}
	end
  end
  ...
  if damageRequest.damageSourceKind == status.statusProperty("damageAllow") then -- IMMUNITY CODE -- (may eventually cook up code to allow for multiple configureable whitelisted damage types, but this will do for now.)
	--damage = 0
  else
	damage = 0
  end
  ...
  local overrideStatResistances = self.overrideStatResistances[elementalStat]
  if overrideStatResistances then --override stats
	--sb.logInfo("override stat resistances")
	resistance = overrideStatResistances
  end
  ...
  if damage > status.statusProperty("damageCap") then damage = status.statusProperty("damageCap") end
