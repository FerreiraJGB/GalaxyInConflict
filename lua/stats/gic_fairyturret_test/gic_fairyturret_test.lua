...
function update(dt)
  ...
  if self.fireTimer == 0 and self.shouldFire == true
	...
	world.spawnProjectile(projectileType, vec2.add(mcontroller.position(), {self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(), self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)}), entity.id(), {1 * self.distPosX, (self.toTargetAngle * 1) + determineInaccuracy()}, false, { power = damagePerShot() })
	self.fireTimer = config.getParameter( "fireRate" )
	end
  end
  ...
  self.tmpoffset = {self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(),self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)}
  animator.translateTransformationGroup("turret",{self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(),self.totalOffsetY +  self.yMuzzleOffset - (1 * self.crouchingQuery)})
  if self.shouldFire == true then
  animator.rotateTransformationGroup("turret", vec2.angle(world.distance(self.canPos, vec2.add(mcontroller.position(), self.tmpoffset))), {self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(),self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)})
  else
  animator.rotateTransformationGroup("turret",vec2.angle(self.toTarget),{self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(), self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)})
  end
  
  animator.resetTransformationGroup("body")
  animator.translateTransformationGroup("body",{self.totalOffsetX + self.xOffset * mcontroller.facingDirection(), self.totalOffsetY + self.yOffset - (1 * self.crouchingQuery)})
  ...
  if self.ammoCount == 0  or
   ...
   world.spawnProjectile("invisibleprojectile", vec2.add(mcontroller.position(), {self.totalOffsetX + self.xMuzzleOffset * mcontroller.facingDirection(), self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)}), entity.id(), {0,0}, false, params)
   self.ammoCount = config.getParameter("ammoCountMax",50)
   self.reloadQuery = false
   end
  end
...
function findTargets()
  local pos = vec2.add(mcontroller.position(),{self.totalOffsetX + self.xMuzzleOffset,self.totalOffsetY + self.yMuzzleOffset - (1 * self.crouchingQuery)})
  ...
  for _, candidate in ipairs(candidates) do
    if world.entityCanDamage(self.sourceEntity, candidate) then
      ...
      if not world.lineTileCollision(pos, canPos) then
		...
		if self.trackingAngle <= 180 then
