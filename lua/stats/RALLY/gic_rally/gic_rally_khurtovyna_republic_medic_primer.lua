...
local duration = 5
function update()
	if status.stat("gic_rally_khurtovyna_republic_medic") == 1 then
		status.addEphemeralEffect("gic_khurtovyna_medic_heal",1); -- this function can config effect duration
		status.addEphemeralEffect("gic_maxhealthboost_floran_200",30); -- this function can config effect duration	
