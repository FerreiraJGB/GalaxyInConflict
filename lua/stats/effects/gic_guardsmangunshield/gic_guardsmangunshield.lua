...
function init()
  ...
  self.statName = "gic_guardsmangunshield" -- name of the stat that gets set to value 1; system only works if this stat value is set to 1, otherwise this effect will be ignored.
  self.nameIdentifier = "guardsman" -- this can be anything, only purpose is to internally store data in format {stat, activated?}
  self.cooldownEffect = "gic_guardsmangunshield_weakness" -- status effect to be triggered after damage is negated/reduced
  self.cooldownEffectDuration = 2 -- duration of cooldown status effect
  self.shouldEntirelyNullifyDamage = false -- if true, then damage is *entirely* nullified when system triggers.
  ...
  self.statModifierGroup = effect.addStatModifierGroup({
	...
	--{stat = "physicalResistance", amount = 2.0},
