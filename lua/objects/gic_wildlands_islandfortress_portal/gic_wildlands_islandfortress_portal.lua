...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Asylum Fortress Grounds",
          planetName = "[Absolution]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_wildlands_islandfortress:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
