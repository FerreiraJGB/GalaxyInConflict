  ...
  if config.getParameter("messagePlayerInterval") then
    self.radioMessage = util.interval(config.getParameter("messagePlayerInterval"), function()
      ...
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "gic_wartorn_flakturm_traitorscave_portal")
      end
    end)
  end
end
...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          name = "He escaped?",
          planetName = "[But to where?]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_wartorn_flakturm_traitorscave:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
