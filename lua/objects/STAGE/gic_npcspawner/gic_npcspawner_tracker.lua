...
function update(dt)
  if storage.stock ~= 0 then
    ...
    elseif storage.cooldown <= 0 and ((not self.trigger) or (self.trigger == "wireunload" and object.getInputNodeLevel(0))) then 
		for i = 0,storage.stock,1 do
			spawn()
		end
		--sb.logInfo(#npcList)
	end
  ...
  local invalids = checkNpcs()
  if invalids >= self.deadTriggerThreshold then
...
function checkNpcs()
	...
	if #npcList > 0 then
		for i = 1,#npcList,1 do
			local validEnt = world.entityPosition(npcList[i])
			if not validEnt then invalids = invalids + 1 end
		end
		--sb.logInfo(invalids)
	end
...
function spawn()
  ...
  while attempts > 0 do
    ...
    if not self.outOfSight or not world.isVisibleToPlayer({spawnPosition[1] - 3, spawnPosition[2] - 3, spawnPosition[1] + 3, spawnPosition[2] + 3}) then
      local typeName = util.randomFromList(self.typeName)
      local npc = util.randomFromList(self.npc)      
	  self.npcParams.level = self.npcLevel and util.randomInRange(self.npcLevel) or world.threatLevel()

      local npcId = world.spawnNpc(spawnPosition, npc, typeName, self.npcParams.level, nil, npcParameter or {})
      if npcId ~= 0 then
		--sb.logInfo(npcId .. " id")
		npcList[#npcList+1] = npcId --start tracking NPCs
		--sb.logInfo(npcList[#npcList])
