  ...
  if config.getParameter("messagePlayerInterval") then
    self.radioMessage = util.interval(config.getParameter("messagePlayerInterval"), function()
      ...
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "gic_psychicrealm_astraltemple_portal")
      end
    end)
  end
end
...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          name = "Astral Temple",
          planetName = "[Wolves of the Abyss]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_psychicrealm_astraltemple:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
