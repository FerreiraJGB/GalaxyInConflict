...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Venerable Ancient Battlefield",
          planetName = "[The Foughten Field]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_daimyohunt_venerableancientbattlefield:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
