...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Factory Laboratory Core",
          planetName = "[Hell's Mantle]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_protectedcolony_hellsmantle:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
