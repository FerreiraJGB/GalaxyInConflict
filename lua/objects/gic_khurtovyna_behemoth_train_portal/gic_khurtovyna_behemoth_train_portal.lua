...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Behemoth Assault",
          planetName = "[Raid the Train | CHOICE]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_behemoth_train:%s:%s", storage.uuid, world.threatLevel())
        },
{
          name = "Station Meeting",
          planetName = "[Negotiate | CHOICE]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_republicmeeting:%s:%s", storage.uuid, world.threatLevel())
        }
