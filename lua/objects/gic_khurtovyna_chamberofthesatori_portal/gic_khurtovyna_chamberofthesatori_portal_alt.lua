...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Chamber of the Satori",
          planetName = "[War Cry of Earth]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_chamberofthesatori_alt:%s:%s", storage.uuid, world.threatLevel())
        }
		}
      }
    }
