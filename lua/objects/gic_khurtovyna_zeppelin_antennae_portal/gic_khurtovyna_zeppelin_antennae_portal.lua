...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Suspicious Antennae",
          planetName = "[Warp to signal destination?]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_zeppelin:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
