  ...
  if config.getParameter("messagePlayerInterval") then
    self.radioMessage = util.interval(config.getParameter("messagePlayerInterval"), function()
      ...
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "gic_frostedsecretline_ghosttrain")
      end
    end)
  end
end
...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "The Secret Line",
          planetName = "[Ghost Train]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_frostedsecretline_ghosttrain:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
