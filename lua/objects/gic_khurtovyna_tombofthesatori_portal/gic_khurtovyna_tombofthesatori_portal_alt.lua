...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Tomb of the Satori",
          planetName = "[Strike of the Living]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_tombofthesatori_alt:%s:%s", storage.uuid, world.threatLevel())
        }
		}
      }
    }
