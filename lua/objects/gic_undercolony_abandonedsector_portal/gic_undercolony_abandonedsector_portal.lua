  ...
  if config.getParameter("messagePlayerInterval") then
    self.radioMessage = util.interval(config.getParameter("messagePlayerInterval"), function()
      ...
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "gic_undercolony_abandonedsector")
      end
    end)
  end
end
  ...
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          ...
          planetName = "[The Lost]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_undercolony_abandonedsector:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
