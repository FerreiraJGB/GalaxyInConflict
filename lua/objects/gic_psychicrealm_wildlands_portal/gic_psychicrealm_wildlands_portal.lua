  ...
  if config.getParameter("messagePlayerInterval") then
    self.radioMessage = util.interval(config.getParameter("messagePlayerInterval"), function()
      ...
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "gic_psychicrealm_wildlands_portal")
      end
    end)
  end
end
...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Extradimensional Realm",
          planetName = "[Wild Entrance | ?феи?]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_psychicrealm_astralplane_wildlands:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
