...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Vault of the Satori",
          planetName = "[Spartan Assault]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_vaultofthesatori_alt:%s:%s", storage.uuid, world.threatLevel())
        }
		}
      }
    }
