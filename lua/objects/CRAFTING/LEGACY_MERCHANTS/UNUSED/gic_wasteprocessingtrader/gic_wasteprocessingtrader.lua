...
function onInteraction(args)
  ...
  addRecipes(storeInventory.alltime, "alltime")
  addRecipes(storeInventory.information, "information")
...
function generateRecipe(itemName, category)
  return {
    input = { {"gic_physicalcredit", math.floor(self.buyFactor * (root.itemConfig(itemName).config.price or root.assetJson("/merchant.config").defaultItemPrice))} },
    output = itemName,
    groups = { category }
  }
end
