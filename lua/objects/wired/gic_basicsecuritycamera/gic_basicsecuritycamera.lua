...
function init()
  ...
  self.targetScanAngle = util.toRadians(config.getParameter("targetScanAngle"))
...
function scanState()
  ...
  local intFoundTarget = false
  local foundTargetTicks = 0

  local scan = coroutine.wrap(function()
    while true do
      local target = findTarget()
      if target then
		foundTargetTicks = math.min(foundTargetTicks + 1,3)
		--return self.state:set(fireState, target) 
	  else
		if foundTargetTicks > 0 then foundTargetTicks = 0 end
	  end
	  
	  if foundTargetTicks == 3 then
		return self.state:set(fireState, target) 
	  end
...
function autoFire()
  ...
  while true do
    ...
    world.spawnProjectile("", firePosition( -1.0, 0.0 ), entity.id(), aimVector, false, projectileParameters)
...
function findTarget()
  ...
  return util.find(nearEntities,  function(entityId)
    ...
    local toTarget = world.distance(targetPosition, self.basePosition)
    local targetAngle = math.atan(toTarget[2], object.direction() * toTarget[1])
    return world.magnitude(toTarget) > self.targetMinRange and math.abs(animator.currentRotationAngle("gun") - (targetAngle)) < self.targetScanAngle and math.abs(targetAngle) < self.targetAngleRange
  end)
end
