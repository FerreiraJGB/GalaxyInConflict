...
function trigger()
  ...
  if config.getParameter("firstProjectile") ~= nil then
    world.spawnProjectile(config.getParameter("firstProjectile"), object.toAbsolutePosition({ 0.5, 0.5 })) --0.2
  end
  ...
  if self.triggerTimer > 0 then
    ...
    if self.triggerTimer == 0 then
      if config.getParameter("secondProjectile") ~= nil then
        world.spawnProjectile(config.getParameter("secondProjectile"), object.toAbsolutePosition({ 0.5, 0.2 }))
      end
      object.smash()
    end
  else
