...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          name = "TRAIN ESCAPE",
          planetName = "[SHIPMENT SECURED]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_khurtovyna_bunkercomplex_trainescape_voidcoreshipment:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
