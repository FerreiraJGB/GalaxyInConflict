...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Ancient Vault's Kiln",
          planetName = "[Mankind's Undying Hope]",
          icon = "default",
          warpAction = string.format("InstanceWorld:gic_ancientvault_kiln:%s:%s", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
