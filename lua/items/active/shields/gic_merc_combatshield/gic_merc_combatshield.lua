...
function raiseShield()
  ...
  if self.knockback > 0 then
    local knockbackDamageSource = {
      poly = shieldPoly,
      damage = (config.getParameter("damage") or 50),
      damageType = (config.getParameter("damageType") or "IgnoresDef"),
      damageSourceKind = (config.getParameter("damageKind") or "ews_melee"),
      --damageKind = "ews_melee",
      sourceEntity = activeItem.ownerEntityId(),
      team = activeItem.ownerTeam(),
      knockback = self.knockback,
      rayCheck = true,
      damageRepeatTimeout = (config.getParameter("damageRepeatTimeout") or 0.5)
    }
