...
function Weapon:new(weaponConfig)
  ...
  newWeapon.ammoAmount = config.getParameter("ammoMax",1) or 1
  --self.weapon.ammoMax = config.getParameter("ammoMax",1) or 1
  --self.weapon.ammoAmount = self.weapon.ammoMax
  newWeapon.altAmmoMax = config.getParameter("altAmmoMax",1) or 1
  newWeapon.altAmmoAmount = config.getParameter("altAmmoAmount") or newWeapon.altAmmoMax
  
  --newWeapon.ammoMax = config.getParameter("ammoMax",1) or 1
  --newWeapon.ammoAmount = config.getParameter("ammoAmount",1) or 1
  --extraStorage = config.getParameter("tooltipFields")
  --extraStorage["ammoAmountLabel"]=newWeapon.ammoAmount.."/"..newWeapon.ammoMax
...
function Weapon:init()
  ...
  self.ammoMax = config.getParameter("ammoMax",1) or 1
  self.ammoAmount = config.getParameter("ammoAmount",1) or 1
  extraStorage = config.getParameter("tooltipFields")
  extraStorage["ammoAmountLabel"]=self.ammoAmount.."/"..self.ammoMax
  activeItem.setInstanceValue("tooltipFields",extraStorage)
...
function Weapon:setStance(stance)
  ...
  --echoutil.logJson(config.getParameter( "npcGun" ))
  --echoutil.logJson(status.stat("ews_npcgun"))
  
  
  --if config.getParameter( "npcGun" ) == true or status.stat("ews_npcgun") == 1 then
  --else
	--activeItem.setInstanceValue("weaponOffsetVar",self.weaponOffset)
	--activeItem.setInstanceValue("weaponRotationVar",self.relativeWeaponRotation)
  --end
  ...
  --if self.laserSightAttached == true then
	...
	--if self.lasersightTransformationGroupWeapon == true then
		--resultLaserSight.angle = self.relativeWeaponRotation or 0
		--local laseroffset = self.weaponOffset or {0,0}
		--resultLaserSight.offset = {laseroffset[1] + resultLaserSight.originalOffset[1],laseroffset[2] + resultLaserSight.originalOffset[2]}
	--end
	
	--echoutil.logJson(self.lasersightTransformationGroupWeapon)
	--echoutil.logJson(resultLaserSight)
  ...
  local returnData = {}
  returnData[1] = self.weaponOffset
  returnData[2] = self.relativeWeaponRotation
  return returnData
end
