...
--	self.weapon.reloading = 0
 -- end
  ...
  --if self.damagePerShot then
  --self.weapon.baseDpsTooltipTemp = self.damagePerShot * (self.baseDamageMultiplier or 1.0) * config.getParameter("damageLevelMultiplier")
  --else                 this small section was for an experiental damage per shot feature, that failed to work. baseDamage might already work right, so this might've been a waste of my time
 ...
 -- end
  ...
  --if self.useRPM == 1 then
  --self.weapon.firerateTooltipTemp = util.round(1 / (self.fireTime or 1.0) * 60, 1)
  --                                                                                  ignore this, already semi-built in with buildscript
  --self.firerateTooltipTemp = config.getParameter("tooltipFields")
  --self.firerateTooltipTemp["speedLabel"]=self.weapon.firerateTooltipTemp
  --activeItem.setInstanceValue("tooltipFields",self.firerateTooltipTemp)
  --end
  
  --RPM stuff (again, experiment. Not worth spending 10 hours on just adding a RPM functionality)
  --if config.getParameter("RPM") == nil then
  self.cooldownTimer = self.fireTime
  --else
  --self.fireTime = (1/(config.getParameter("RPM") / 60))
  --self.cooldownTimer = self.fireTime
  --end
...
function GunFire:damagePerShot()
  --if self.damagePerShot then
  --return ((self.damagePerShot)+self.bonusDps) * (self.baseDamageMultiplier or 1.0) * config.getParameter("damageLevelMultiplier") / self.projectileCount
  --else
