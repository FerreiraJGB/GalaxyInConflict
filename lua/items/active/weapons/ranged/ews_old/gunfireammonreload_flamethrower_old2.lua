...
function GunFire:update(dt, fireMode, shiftHeld)
	...
	--main flamethrower bit that determines when/when not to have flamethrower sfx
...
function GunFire:fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
  ...
  if self.missChanceToggle == true then
  self.missChanceRoll = self.missChanceVar * (1 + (self.dynamicRecoilMissMultiplier) * self.dynamicRecoilSteps)
  end
  ...
  for i = 1, (projectileCount or self.projectileCount) do
	...
	projectileId = world.spawnProjectile(
        ...
        firePosition or self:firePosition(),
        activeItem.ownerEntityId(),
        self:aimVector((inaccuracy or self.inaccuracy) * (1 + (self.dynamicRecoilMutiplier) * self.dynamicRecoilSteps)),
        false,
        params
      )
