...
function GunFire:init()
  ...
  -- no longer needed, this is automatically done in the crouch update scripts
...
function GunFire:update(dt, fireMode, shiftHeld)
    ...
    and self.cooldownTimer == 0 then
		...
		elseif self.changeFireModeHoldTimer == 0 then
			self.changeFireModeHoldTimer = 0.1--player has to hold down alt for 0.2 seconds to properly reload. Done to allow other alt-based abilities to exist.
...
function GunFire:auto()
  ...
  if self.npcGun == true then
	self.cooldownTimer = self.fireTime
	if not status.stat("ews_npcfirerate") == 0 then
...
function GunFire:reload()
	...
	if not self.singleBulletLoad == true or not config.getParameter( "partialReloadsEnabled" ) == true then
		animator.playSound("switchAmmo")
	end
		...
		if config.getParameter( "partialReloadsEnabled" ) == true then
			if self.weapon.ammoAmount > 0 then						--partial reload
				...
				if self.stances.partreload.duration then
					...
					world.spawnProjectile(self.magazineProjectilePartial, firePosition or self:firePosition(), activeItem.ownerEntityId(),    self:aimVector(inaccuracy or self.inaccuracy),false,params)
				end
