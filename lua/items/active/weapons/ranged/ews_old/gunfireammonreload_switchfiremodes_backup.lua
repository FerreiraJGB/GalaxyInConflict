...
function GunFire:fireProjectileSwitch()
    ...
    if self.fireType == self.fireTypeTemp then
			...
			params.power = (((self.baseDamage or (self.baseDps * self.fireTime)))*(1.0)*config.getParameter("damageLevelMultiplier"))
			world.spawnProjectile(projectileType, self:firePosition(), activeItem.ownerEntityId(), self:aimVector(self.altInaccuracy),false,params)
	end
...
function GunFire:burst()
  ...
  self.cooldownTimer = (self.fireTime - self.burstTime) * self.burstCount + self.burstCooldown
  animator.setParticleEmitterActive("muzzleFlash", false)
  animator.setParticleEmitterActive("hotsmoke", false)
end
