...
function GunFire:init()
  ...
  if config.getParameter( "attachment_sight" ) then
	animator.setGlobalTag("sightImage", config.getParameter( "attachment_sight_image" ))
  end
  if config.getParameter( "attachment_barrel" ) then
	animator.setGlobalTag("barrelImage", config.getParameter( "attachment_barrel_image" ))
  end
  if config.getParameter( "attachment_stock" ) then
	animator.setGlobalTag("stockImage", config.getParameter( "attachment_stock_image" ))
  end
end
...
function GunFire:setFinalVariables()
	self.finalInaccuracy = self.inaccuracy
	self.finalInaccuracyCrouch = (self.crouchInaccuracy or self.finalInaccuracy * 0.4)
	self.finalMissChance = self.missChance
	self.finalMissChanceCrouch = (self.missChanceCrouch or self.finalMissChance * 0.4)
	self.finalDynamicRecoilMaxSteps = self.dynamicRecoilMaxSteps
	self.finalDynamicRecoilMultiplier = self.dynamicRecoilMultiplier
	self.finalDynamicRecoilTickDuration = self.dynamicRecoilTickDuration
	self.finalDynamicRecoilMissMultiplier = self.dynamicRecoilMissMultiplier
  ...
  if config.getParameter( "attachment_sight_bonusType" ) then
	local attachmentBonusType = config.getParameter( "attachment_sight_bonusType" )
	--sb.logInfo(sb.printJson(attachmentBonusType))
	local attachmentBonuses = config.getParameter( "attachment_sight_bonuses" )
  ...
  if config.getParameter( "attachment_barrel_bonusType" ) then
	local attachmentBonusType = config.getParameter( "attachment_barrel_bonusType" )
	--sb.logInfo(sb.printJson(attachmentSightBonusType))
	local attachmentBonuses = config.getParameter( "attachment_barrel_bonuses" )
	--sb.logInfo(sb.printJson(attachmentBonuses[1]))
  ...
  if config.getParameter( "attachment_stock_bonusType" ) then
	local attachmentBonusType = config.getParameter( "attachment_stock_bonusType" )
	--sb.logInfo(sb.printJson(attachmentSightBonusType))
	local attachmentBonuses = config.getParameter( "attachment_stock_bonuses" )
  ...
  if config.getParameter( "attachment_sight_bonusesAlt" ) then
	local attachmentBonusType = config.getParameter( "attachment_sight_bonusType" )
	local attachmentBonusesAlt = config.getParameter( "attachment_sight_bonusesAlt" )
  ...
  if config.getParameter( "attachment_barrel_bonusesAlt" ) then
	local attachmentBonusType = config.getParameter( "attachment_barrel_bonusType" )
	local attachmentBonusesAlt = config.getParameter( "attachment_barrel_bonusesAlt" )
  ...
  if config.getParameter( "attachment_stock_bonusesAlt" ) then
	local attachmentBonusType = config.getParameter( "attachment_stock_bonusesAlt" )
	local attachmentBonusesAlt = config.getParameter( "attachment_stock_bonusesAlt" )
...
function GunFire:damagePerShot()
  ...
  return ((self.damageUsed)+self.bonusDps) * (self.baseDamageMultiplier or 1.0) * config.getParameter("damageLevelMultiplier") / self.projectileCount
  --end
end
