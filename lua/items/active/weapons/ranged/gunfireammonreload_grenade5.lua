...
function GunFire:update(dt, fireMode, shiftHeld)
 ...
 if shiftHeld and (self.weapon.ammoAmount < config.getParameter("ammoMax",1)) then
  ...
  if self.cookTimer > 3
  and self.cookTimer < 4
  and not self.tick4 == true
  and self.pinPulled == true then
  animator.playSound("tick")
  self.tick4 = true
  end
  ...
  if self.cookTimer == 0
  and self.pinPulled == true then
  world.spawnProjectile(self.projectileType, mcontroller.position(), activeItem.ownerEntityId(), {0, 0}, false, { timeToLive = 0, power = self:damagePerShot() })
  self.pinPulled = false
  item.consume(1)
  self.tick3 = false
  self.tick2 = false
...
function GunFire:auto()
    ...
    self:fireProjectile()
  --animator.setPartTag("muzzleFlash", "variant", math.random(1, 5))
