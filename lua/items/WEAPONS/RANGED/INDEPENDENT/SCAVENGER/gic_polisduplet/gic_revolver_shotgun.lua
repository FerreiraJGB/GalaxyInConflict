...
RevolverShotgun = GunFire:new()

function RevolverShotgun:init()
  GunFire.init(self)
  
  if (not config.getParameter("spentCasings")) then 
	self.spentCasings = jarray();
  else 
	self.spentCasings = config.getParameter("spentCasings")
	-- sb.logInfo(sb.printJson(config.getParameter("spentCasings"))) 
  end
end

function RevolverShotgun:update(dt, fireMode, shiftHeld)
  GunFire.update(self, dt, fireMode, shiftHeld)
end

function RevolverShotgun:auto()
...
function RevolverShotgun:singleload() --SINGLE LOAD RELOAD ANIMS
...
function RevolverShotgun:uninit()
	GunFire.uninit(self)
end