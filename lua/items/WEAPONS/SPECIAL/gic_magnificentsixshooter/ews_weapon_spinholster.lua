...
function BigIron:update(dt, fireMode, shiftHeld)
  GunFire.update(self, dt, fireMode, false)
  ...
  if shiftHeld and self.weapon.ammoAmount < self.weapon.ammoMax 
 ...
 if (self.weapon.ammoAmount == 0) and (self.weapon.firingquery == 0) and self.fireMode == "alt" 
 ...
 if (self.fireMode == (self.activatingFireMode or self.abilitySlot) or self.fireMode == "alt" or shiftHeld)
  ...
  self.altHeld = (self.fireMode == "alt")
end
...
function BigIron:aimReady()
  ...
  util.wait(99999, function()
	mcontroller.controlModifiers({runningSuppressed = true})
    if not self.altHeld == true then return true end
  end)
	...
	world.spawnProjectile("gic_magnificentsixshooter_shot", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.0),false,{power = 0})
...
function BigIron:aimCooldown()
  if self.fireMode == "alt" and (self.weapon.ammoAmount > 0) then
	self:setState(self.aimFire)
  else
	mcontroller.controlModifiers({runningSuppressed = false})
	local progress = 0
