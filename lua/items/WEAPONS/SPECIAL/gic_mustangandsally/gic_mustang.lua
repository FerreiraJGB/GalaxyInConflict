...
function BigIron:aimReady()
  ...
  self.cooldownTimer = 0
	...
	world.spawnProjectile("gic_wallpiercingbullet_c_usoa50", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.0),false,{power = 39.6})
...
function BigIron:aimFire()
  ...
  if self.shiftHeldA == true then
	...
	--	self:fireProjectile("gic_blackgambit_shot",params,0.01,fireOffset,_,true)	
		world.spawnProjectile("gic_wallpiercingbullet_c_usoa50", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.0),false,{power = 55}) -- Notable effect: AimReady fires the very first shot, whereas this one firest subsequent shots. Untested as to what causes it to revert to the "first shot" state, but likely to be related to chain-firing.
