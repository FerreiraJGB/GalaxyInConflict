...
function Charge:update(dt, fireMode, shiftHeld)
  ...
  if self.weapon.currentAbility == nil
	...
	and status.stat("gic_hiltbash_cooldown") == 0
...
function Charge:charge()
	status.addEphemeralEffect("gic_hiltbash_cooldown",40)
	status.addEphemeralEffect("gic_weapon_warcry",7)
	...
	if mcontroller.onGround() and not mcontroller.zeroG() then 
		...
		mcontroller.setXVelocity(mcontroller.facingDirection()*20) --200
	end
