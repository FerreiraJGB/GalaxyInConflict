...
function Parry:init()
  ...
  self.shells = config.getParameter("shells",1)
  self.shellsMax = 1
  
  self.ammoItem = "gic_20mm_musketballshot"
...
function Parry:update(dt, fireMode, shiftHeld)
  ...
  if self.shellLoaded == true then
	activeItem.setCursor("/cursors/spearcursors/lanceload" .. self.shells ..".cursor")
  else
	activeItem.setCursor("/cursors/spearcursors/lanceunload" .. self.shells ..".cursor")
  end
...
function Parry:gunStance()
	...
	animator.playSound("gunFire")
	local fireOffset = vec2.add(mcontroller.position(), activeItem.handPosition({3.3,0.25}))
	...
	world.spawnProjectile("gic_handcannon_immediate_a", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.05),false,{power = 200, actionOnReap = actionOnReapA})
	--for i=1,5,1 do
		world.spawnProjectile("gic_handcannon_b", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.01),false,params)
	--end
...
function Parry:pumpSpear()
	...
	world.spawnProjectile("gic_null",offset,activeItem.ownerEntityId(),{-0.1*mcontroller.facingDirection(),1},false,params)
