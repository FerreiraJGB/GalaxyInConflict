...
function Charge:charge()
	status.addEphemeralEffect("gic_superarmor", self.stances.windup.duration + 0.10 +  self.stances.fire.duration + 0.2)
	...
	if mcontroller.onGround() and not mcontroller.zeroG() then 
		mcontroller.setVelocity({mcontroller.facingDirection()*0,0}) 
  ...
  world.spawnProjectile(
      "gic_battleballad_field_groupheal_celestialsword",
      ...
      {}
		...
		mcontroller.setXVelocity(mcontroller.facingDirection()*0) --200
	end
