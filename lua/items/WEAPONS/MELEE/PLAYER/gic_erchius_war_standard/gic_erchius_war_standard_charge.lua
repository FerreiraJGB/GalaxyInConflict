...
function Charge:update(dt, fireMode, shiftHeld)
  ...
  if self.weapon.currentAbility == nil
	...
	and status.stat("gic_equipmentsummon_cooldown") == 0
...
function Charge:charge()
	...
	status.addEphemeralEffect("gic_equipmentsummon_cooldown",60)
	...
	animator.playSound("heavyAttackSwing")
		  ...
		  world.spawnProjectile(
			"gic_magicshot_erchius_bannerbearer_summon_stage2_gladiator_warbanner_player",
	...
	if mcontroller.onGround() and not mcontroller.zeroG() then 
		...
		mcontroller.setXVelocity(mcontroller.facingDirection()*100) --200
	end
