...
function MeleeCombo:wait()
  ...
  self.weapon:setStance(stance)
  local altAbilityType = config.getParameter("altAbilityType")
  ...
  util.wait(stance.duration, function()
	...
	if altAbilityType == "parry" and (self.shiftHeld or self.fireMode == "alt") then
...
function MeleeCombo:fire()
  ...
  if self.comboStep == 1 then
	...
	--world.spawnProjectile("gic_null", vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})), activeItem.ownerEntityId(),{0,0},false,params)
	
	local damagePositions = {}
	damagePositions[1] = vec2.add({0,0},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[2] = vec2.add({1,0},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[3] = vec2.add({1,1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[4] = vec2.add({0,1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[5] = vec2.add({-1,1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[6] = vec2.add({-1,0},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[7] = vec2.add({-1,-1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[8] = vec2.add({0,-1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	damagePositions[9] = vec2.add({1,-1},vec2.add(mcontroller.position(),activeItem.handPosition({1.8,0.4})))
	
	world.damageTiles(damagePositions, "foreground", mcontroller.position(), "blockish", 20)
  end
