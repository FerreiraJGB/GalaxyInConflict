...
function MeleeCombo:windup()
  ...
  if stance.hold then
	...
	util.wait(stance.duration, function() mcontroller.controlModifiers({runningSuppressed = true}) end)
  end
...
function MeleeCombo:fire()
  ...
  if self.comboStep == 1 or self.comboStep == 4 then
	mcontroller.setVelocity({mcontroller.facingDirection()*20,5})
  elseif self.comboStep == 2 or self.comboStep == 3 then
	mcontroller.setVelocity({mcontroller.facingDirection()*25,0})
  end
