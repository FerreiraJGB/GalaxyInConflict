...
function Charge:charge()
	...
	if mcontroller.onGround() and not mcontroller.zeroG() then 
		mcontroller.setVelocity({mcontroller.facingDirection()*100,0}) 
	...
	animator.setAnimationState("swoosh", "fire9")
	...
	if mcontroller.onGround() and not mcontroller.zeroG() then 
		...
		mcontroller.setXVelocity(mcontroller.facingDirection()*50) --200
	end
