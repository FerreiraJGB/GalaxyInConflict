...
function Parry:gunStance()
	...
	--actionOnReapA[2] = {time = 0.015, action = "particle", rotate = true, specificiation = {type = "textured", image = "/particles/fog/1.png", position = {-1, 0}, layer = "front", timeToLive = 20, destructionTime = 3, destructionAction = "shrink", fullbright = false}}
	--actionOnReapA[3] = actionOnReapA[2]
	
	--world.spawnProjectile("gic_handcannon_immediate_a", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.05),false,{power = 100, actionOnReap = actionOnReapA})
	for i=0,5,1 do
		world.spawnProjectile("gic_redtracerbullet_heavy_c_nobleed", fireOffset,activeItem.ownerEntityId(),self:aimVector(0.05),false,params)
	end
...
function Parry:reloadSpear()
	...
	until self.fireMode == "alt" or self.fireModeTmp == 1 or self.shells == self.shellsMax or not player.hasItem({name = self.ammoItem})
