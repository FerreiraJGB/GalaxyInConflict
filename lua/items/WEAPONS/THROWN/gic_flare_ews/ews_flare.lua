...
function GunFire:init()
  ...
  --self.tick2 = false
  --self.tick1 = false
  self.flareTimerMax = config.getParameter( "flareTimerMax", 60 )
  animator.setLightActive("flare", false)
  
  if config.getParameter( "pinPull" ) == true then
  self.pinPulled = true
  self.cookTimer = config.getParameter( "flareTimer" )
  animator.setLightActive("flare", true)
  animator.setAnimationState("gunState","primed")
  else
  activeItem.setInstanceValue("flareTimer",0)
  activeItem.setInstanceValue("pinPull",false)
...
function GunFire:update(dt, fireMode, shiftHeld)
  ...
  --old code, leaving in only for future reference n etc.
  --if not self.pinPulled == true then
		--activeItem.setCursor()
	--else
		--if self.cookTimer == self.flareTimerMax then
			--activeItem.setCursor("/cursors/aim_ews0.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.875 then
			--activeItem.setCursor("/cursors/aim_ews1.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.75 then
			--activeItem.setCursor("/cursors/aim_ews2.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.625 then
			--activeItem.setCursor("/cursors/aim_ews3.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.5 then
			--activeItem.setCursor("/cursors/aim_ews4.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.375 then
			--activeItem.setCursor("/cursors/aim_ews5.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.25 then
			--activeItem.setCursor("/cursors/aim_ews6.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.125 then
			--activeItem.setCursor("/cursors/aim_ews7.cursor")
		--elseif self.cookTimer/self.flareTimerMax > 0.0 or self.weapon.cookTimer == 0 then
 ...
 --end
  ...
  -- note -- the durability parameter indicates how much durability as a whole an item has.
  -- the durabilityHit parameter is subtracted from the durability parameter to get the "true" durability as shown under the item in a hotbar
  if self.cookTimer > 0 and self.pinPulled == true then
	activeItem.setInstanceValue("durabilityHit",self.cookTimer)
  else
	activeItem.setInstanceValue("durabilityHit",self.flareTimerMax)
  end
  ...
  self.pinPulled = false
  activeItem.setInstanceValue("pinPull",self.pinPulled)
  item.consume(1)
  ...
  --force reload when empty
  if not self.weapon.currentAbility and self.weapon.ammoAmount == 0 then
  ...
  and self.pinPulled == false then
	self.pinPulled = true
	activeItem.setInstanceValue("pinPull",self.pinPulled)
	self.cookTimer = self.flareTimerMax
  ...
  if self.cookTimer > 0 then
  activeItem.setInstanceValue("flareTimer",self.cookTimer)
  end
