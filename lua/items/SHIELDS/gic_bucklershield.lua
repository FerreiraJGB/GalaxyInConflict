...
function lowerShield()
  ...
  activeItem.setItemShieldPolys({})
  activeItem.setItemDamageSources({})
  self.cooldownTimer = config.getParameter("lowerCooldownTime",0.5)
  
  status.modifyResource("perfectBlock", math.max(0,self.perfectBlockTime - status.resource("perfectBlock")))
  status.modifyResource("perfectBlockLimit", math.max(0,1.0 - status.resource("perfectBlockLimit")))
end
